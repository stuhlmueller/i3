...
def parse(s):
  """Turn a string into a list of numbers describing a Bayes net (UAI format)."""
  ...
  toplevel = (net_type +
              parser.oneplus(number | ignored_whitespace) +
              end)
  tokens = string_to_tokens(s)
  return remove_ignored(toplevel.parse(tokens))


def evaluate(rng, stack):
  """Turn random state and stack of (UAI) numbers into Bayesian network."""
  net = bayesnet.BayesNet(rng=rng)
  num_vars = stack.pop(0)
  for i in range(num_vars):
    domain_size = stack.pop(0)
...
def network_from_string(s, rng):
  """Parse (UAI) string to network."""
  number_stack = parse(s)
  net = evaluate(rng, number_stack)
  return net  
